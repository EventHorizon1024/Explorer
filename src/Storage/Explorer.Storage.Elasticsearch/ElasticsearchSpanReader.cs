...
using Explorer.Storage.Elasticsearch.Models;
...
namespace Explorer.Storage.Elasticsearch
{
    public class ElasticsearchSpanReader : ISpanReader
    {
        ...
        public async Task<string[]> GetSeriesAsync()
        {
            ...
            return response.FromJson<QueryResult<object>>().Aggregations["serviceName"].Keys.ToArray();
        }
        ...
        public async Task<string[]> GetOperationsAsync(string serviceName)
        {
            ...
            var response = await httpResponseMessage.Content.ReadAsStringAsync();
            return response.FromJson<QueryResult<object>>().Aggregations["operationName"].Keys.ToArray();
        }
        ...
        {
            var index = $"{{\"index\":\"{_options.IndexName}\"}}";
            ...
            var response = await httpResponseMessage.Content.ReadAsStringAsync();
            var result = response.FromJson<MultiQueryResult<Span>>();
        ...
        private async Task<string[]> FindTraceIDsAsync(TraceQueryParameters query)
        {
            ...
            var response = await httpResponseMessage.Content.ReadAsStringAsync();
            var result = response.FromJson<QueryResult<object>>();
